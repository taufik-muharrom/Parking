        ...
        console.log(durasi+" detik");
          if (item.vehicle == "car"){
            if(durasi <= 3600){
              ...
              console.log('bayar parkir '+bayar1);
              }
            ...
            if(durasi <= 3600){
              ...
              bayar1 = motorJamPertama + ((durasi/3600)*motorJamKedua);
            }
            else{
              return;
            }
            ...
            localStorage.setItem("pendapatan_op1",JSON.stringify(pendapatan1));
...
Durasi parkir ${durasi} detik (${durasi/60} menit) 
  ...
  function selectCode1(){
    ...
    getDataParking.forEach((item)=>{
      ...
      <option id="option">${item.gen_code}</option>
    `

    if (statusCode1 == false){
      setTimeout(deleteCode(),1000);
      setTimeout(statusCode1=true,1000);
      }

      function deleteCode(){
        item.splice(index,1);
        localStorageData.setItem("data_parking1",JSON.parse(item))
      };      
    })
        ...
        console.log(durasi+" detik");
          if (item.vehicle == "car"){
            if(durasi <= 3600){
              ...
              console.log('bayar2 parkir '+bayar2);
              }
          if(item.vehicle == "motorcycle"){
            if(durasi <= 3600){
              ...
              bayar2 = motorJamPertama + ((durasi/3600)*motorJamKedua);
            }
            else{
              return;
            }
            ...
            localStorage.setItem("pendapatan_op2",JSON.stringify(pendapatan2));
